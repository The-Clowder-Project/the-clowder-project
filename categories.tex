...
\begin{Proof}{Proof of \cref{properties-of-natural-transformations-associated-to-functors}}%
    ...
    By \cref{properties-of-natural-isomorphisms-characterisations}, \cref{properties-of-natural-isomorphisms}, $F^\dagger$ is a natural isomorphism if each $F^\dagger_{A,B} \defeq F_{A,B}$ is an isomorphism in $\Sets$. But that means each $F_{A,B}$ is bijective, i.e. injective and surjective. By \cref{faithful-functors} and \cref{full-functors}, this is the case iff $F$ is fully faithful.
    ...
    We have
    \begin{align*}
      (G \circ F)^\dagger_{A,B}(f) &= (G \circ F)_{A,B}(f)\\
                                  &= G_{F(A),F(B)}(F_{A,B}(f))\\
                                   &=  G^\dagger_{F^{\op}(A),F(B)}(F_{A,B}(f))\\
                                   &=  (G^\dagger\twocirc \id_{F^{\op}\times F})_{A,B} (F_{A,B}(f))\\
                                   &=  ((G^\dagger\twocirc \id_{F^{\op}\times F}) \circ F^\dagger)_{A,B}(f).\\
    \end{align*}
    Hence the two natural transformation must be identical.
    ...
    The component $\idfun^\dagger_{\CatFont{C}, A,B}$ for $A,B \in \CatFont{C}$ is given by $\idfun_{\CatFont{C}, A,B} \colon \Hom_{\CatFont{C}}(A,B) \to \Hom_{\CatFont{C}}(A,B)$, which acts as the identity. Hence $\idfun^\dagger$ is indeed the identity natural transformation.
\end{Proof}

...
\begin{proposition}{Properties of Pre/Postcomposition}{properties-of-pre-postcomposition}%
    ...
    \begin{enumerate}
        \item\label{properties-of-pre-postcomposition-interaction-between-precomposition-and-postcomposition}\SloganFont{Interaction Between Precomposition and Postcomposition. }We have
            \begin{webcompile}
                g_{*}\circ f^{*}%
                =%
                f^{*}\circ g_{*},%
                ...
                \end{tikzcd}
            \end{webcompile}%
        \item\label{properties-of-pre-postcomposition-interaction-with-composition-1}\SloganFont{Interaction With Composition \rmI. }We have
            \begin{webcompile}
              \begin{aligned}
                ...
                \end{aligned}
            \end{webcompile}%
        \item\label{properties-of-pre-postcomposition-interaction-with-composition-2}\SloganFont{Interaction With Composition \rmII. }We have
            \begin{webcompile}
                ...
                \end{tikzcd}
            \end{webcompile}%
        \item\label{properties-of-pre-postcomposition-interaction-with-composition-3}\SloganFont{Interaction With Composition \rmIII. }Now let $f \colon X \to A, g \colon C \to D$ in $\CatFont C$. Then we have
            \begin{envfootnotesize}
                \begin{webcompile}
                    \begin{aligned}
                        f^{*}\circ\circ^{\CatFont{C}}_{A,B,C}
                        &=
                        \circ^{\CatFont{C}}_{X,B,C}\circ(\sfid \times f^{*}),%
                        %
                        ...
                        &=
                        \circ^{\CatFont{C}}_{A,B,D}\circ(g_{*} \times \sfid),%
                        %
                        \begin{tikzcd}[row sep={5.0*\the\DL,between origins}, column sep={12.0*\the\DL,between origins}, background color=backgroundColor, ampersand replacement=\&]
                            \Hom_{\CatFont{C}}(B,C)\times\Hom_{\CatFont{C}}(A,B)
                            \arrow[r,"\circ^{\CatFont{C}}_{A,B,C}"]
...
\begin{Proof}{Proof of \cref{properties-of-pre-postcomposition}}%
    ...
    It suffices to show the equalities of the maps on $\point \in \pt$:
    We have
    \begin{align*}
      [g \circ f](\point) &= g \circ f \\
                          &= g_*(f) \\
                          &= g_*([f](\point)) = (g_* \circ [f])(\point),
      [g \circ f](\point) &= g \circ f \\
                          &= f^*(g) \\
                          &= f^*([g](\point)) = (f^* \circ[g])(\point).
    \end{align*}
    \ProofBox{\cref{properties-of-pre-postcomposition-interaction-with-composition-3}: Interaction With Composition \rmIII}%
    We show the two equations pointwise on $(h,k) \in \Hom_{\CatFont C}(B,C) \times \Hom_{\CatFont C}(A,B)$.
    \begin{enumerate}
      \item We have
            \begin{align*}
              (f^* \circ \circ^{\CatFont C}_{A,B,C})(h,k)
                &= f^* (h \circ k)\\
                &= (h \circ k) \circ f\\
                &= h \circ (k \circ f)\\
                &= \circ^{\CatFont C}_{X,B,C}(h, k \circ f)\\
                &= \circ^{\CatFont C}_{X,B,C}(h, f^*(k))\\
                &= (\circ^{\CatFont C}_{X,B,C} \circ (\sfid \times f^*))(h,k).
            \end{align*}
      \item We have
            \begin{align*}
              (g_* \circ \circ^{\CatFont C}_{A,B,C})(h,k)
                &= g_* (h \circ k)\\
                &= g \circ (h \circ k)\\
                &= (g \circ h) \circ k\\
                &= \circ^{\CatFont C}_{A,B,D}(g \circ h, k)\\
                &= \circ^{\CatFont C}_{A,B,D}(g_*(h), k)\\
                &= (\circ^{\CatFont C}_{A,B,D} \circ (g_* \times \sfid))(h,k).
            \end{align*}
            \end{align*}
    \end{enumerate}

    \ProofBox{\cref{properties-of-pre-postcomposition-interaction-with-identities}: Interaction With Identities}%
    For $h \in \Hom_{\CatFont C}(A,B)$ we have
    \[ (\sfid_A)^*(h) = h \circ \sfid = h = \sfid_{\Hom_{\CatFont C}(A,B)}(h) \]
    and
    \[ (\sfid_B)_*(h) = \sfid \circ h = h = \sfid_{\Hom_{\CatFont C}(A,B)}(h). \]
    Hence the maps are identical.
\end{Proof}
...
\begin{Proof}{Proof of \cref{properties-of-posetal-categories}}%
  ...
  \begin{enumerate}
    \item We construct a functor $f_\pos \colon X_\pos \to Y_\pos$ from a monotone map $f \in \Hom_\Pos(X,Y)$ by
          \[ f_\pos(a) \defeq f(a), f_\pos(\pt_{\Hom_{X_\pos}(a,b)}) \defeq \pt_{\Hom_{Y_\pos}(f(a),f(b)}\] for any $a,b \in X$.
          This map is well-defined, since $a \preceq_X b$ implies $f(a) \preceq_Y f(b)$, hence $\pt$ is an element of $\Hom_{Y_\pos}(f(a),f(b)}$ if it is one of $\Hom_{X_\pos}(a,b)$.
          $f_\pos$ is indeed a functor: It preserves identities and compositions trivially, since all morphisms in $\Hom$-sets of $Y_\pos$ are equal.
    \item The constructions $X \mapsto X_\pos$ for any poset $(X, \preceq_X)$, $f \mapsto f_\pos$ for any $f \in \Hom_\Pos(X,Y)$ define the functor
          \[
                (-)_{\pos}%
                \colon%
                \Pos%
                ...
                \Cats.
          \]
          Indeed, this mapping preserves identities and compositions:
          \begin{itemize}
            \item For $X$ a poset, $a,b \in X, a \preceq_X b$, we have
                  \[ (\sfid_X)_\pos(a) = \sfid_X(a) = \sfid_{X_\pos}(a),\]
                  so $(\sfid_X)_\pos$ acts like the identity functor of $X_\pos$ on objects, and
                  \[ (\sfid_X)_\pos(\pt_{\Hom_{X_\pos}(a,b)}) = \pt_{\Hom_{X_\pos}((\sfid_X)_\pos(a),(\sfid_X)_\pos(b))} = \pt_{\Hom_{X_\pos}(a,b)},\]
                  so the same holds for morphisms and $(\sfid_X)_\pos = \sfid_{X_\pos}$.
            \item For $X,Y,Z$ posets, $f \in \Hom_\Pos(X,Y), g \in \Hom_\Pos(Y,Z)$, we need to show
                  \[ (g \circ f)_\pos = g_\pos \circ f_\pos. \]
                  Let $a, b \in X, a \preceq_X b$. Then we have
                  \begin{align*}
                    (g \circ f)_\pos(a) &= (g \circ f)(a)\\
                                        & = g(f(a)) = g_\pos(f_\pos(a)),
                  \end{align*}
                  so the identity holds on objects.
                  Since $Z_\pos$ is a posetal category, the identity automatically holds on morphisms since
                  \[(g \circ f)_\pos(\pt_{\Hom_{X_\pos}(a,b)}) = \pt_{\Hom_{Z_\pos}(g_\pos(f_\pos(a)), g_\pos(f_\pos(b)))} = (g_\pos \circ f_\pos)(\pt_{\Hom_{X_\pos}(a,b)}).\]
          \end{itemize}
  \end{enumerate}
    Hence $(-)_{\pos}$ is indeed a functor.
...
\begin{Proof}{Proof of \cref{properties-of-equivalences-of-categories}}%
    ...
    \begin{enumerate}
        \item\SloganFont{\cref{properties-of-equivalences-of-categories-characterisations-a}$\implies$\cref{properties-of-equivalences-of-categories-characterisations-b}: }Clear.

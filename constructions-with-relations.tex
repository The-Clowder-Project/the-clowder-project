...
\begin{enumerate}
    ...
    \item\label{constructions-with-relations-introduction-item-2}The various kinds of constructions involving relations, such as graphs, domains, images, unions, intersections, products, converse relations, composition of relations, and collages (\cref{section-more-constructions-with-relations}).
\end{enumerate}
...
\subsection{The Domain and Range of a Relation}\label{subsection-the-domain-and-image-of-a-relation}
Let $A$ and $B$ be sets.
\begin{definition}{The Domain and Range of a Relation}{the-domain-and-image-of-a-relation}%
    Let $R\colon A\rightproarrow B$ be a relation.%
    %--- Begin Footnote ---%
    \footnote{%
        Following \ChapterRef{\ChapterProfunctors, \cref{profunctors:the-domain-and-image-of-a-profunctor}}{\cref{the-domain-and-image-of-a-profunctor}}, we may compute the (characteristic functions associated to the) domain and image of a relation using the following colimit formulas:
        \begin{align*}
            \chi_{\Dom(R)}(a) &\cong \colim_{b\in B}(R^{b}_{a})\qquad(a\in A)\\
                              &\cong \bigvee_{b\in B}R^{b}_{a},\\
            \chi_{\Im(R)}(b)  &\cong \colim_{a\in A}(R^{b}_{a})\qquad(b\in B)\\
    ...
    \begin{enumerate}
        \item\label{the-domain-of-a-relation}The \index[set-theory]{relation!domain of}\textbf{domain of $R$} is the subset \index[notation]{domR@$\Dom(R)$}$\Dom(R)$ of $A$ defined by
            \[
                ...
                \}.%
            \]%
        \item\label{the-image-of-a-relation}The \index[set-theory]{relation!image of}\textbf{image of $R$} is the subset \index[notation]{ImR@$\Im(R)$}$\Im(R)$ of $B$ defined by
            \[
                \Im(R)%
                \defeq%
                \{%

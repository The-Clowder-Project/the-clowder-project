...
\begin{definition}{The 2-Category of Relations With Apartness Composition}{the-2-category-of-relations-with-apartness-composition}%
    ...
    \end{itemize}
\end{definition}
\begin{proposition}{2-Isomorphism Between $\sfbfRel$ and $\aptsfbfRel$}{2-isomorphism-between-sfrel-and-sfaptrel}%
    The functor
    \[
        (-)^{\sfc}%
        \colon%
...
\begin{lemma}{Conditions Involving a Relation and Its Converse \rmII}{conditions-involving-a-relation-and-its-converse-2}%
    Let $R^{\sfc}=B\times A\setminus R$. The conditions below are row-wise equivalent:
    \begingroup%
    \setlength\cellspacetoplimit{3pt}
    \setlength\cellspacebottomlimit{3pt}
    \renewcommand{\arraystretch}{1.2}
    \begin{center}
        \begin{tabular}{|Sc|Sc|}\\\hline\rowcolor{darkRed}
            \textcolor{white}{\textbf{\textsc{Condition}}} & \textcolor{white}{\textbf{\textsc{Inclusion}}}       \\\hline\rowcolor{backgroundColor}
            $R^{\sfc}$ is functional                       & $R\aptcirc R^{\dagger}\subset \nabla_{B}$            \\\rowcolor{backgroundColor}
            $R^{\sfc}$ is surjective                       & $\nabla_{A}           \subset R^{\dagger}\aptcirc R$ \\\rowcolor{black!05!backgroundColor}
            $R^{\sfc}$ is total                            & $\nabla_{B}           \subset R\aptcirc R^{\dagger}$ \\\rowcolor{backgroundColor}
            $R^{\sfc}$ is injective                        & $R^{\dagger}\aptcirc R\subset \nabla_{A}$            \\\hline
        \end{tabular}
    \end{center}
    \endgroup
\end{lemma}
\begin{Proof}{Proof of \cref{conditions-involving-a-relation-and-its-converse-2}}%
    This follows from \cref{conditions-involving-a-relation-and-its-converse-1} and \cref{properties-of-apartness-composition-of-relations-relation-to-composition-with-relations} of \cref{properties-of-apartness-composition-of-relations}. Still, here we note what each statement unwinds to:
    \begingroup%
    \setlength\cellspacetoplimit{3pt}
    \setlength\cellspacebottomlimit{3pt}
    \renewcommand{\arraystretch}{1.2}
    \begin{center}
        \begin{tabular}{|Sc|Sc|Sc|}\\\hline\rowcolor{darkRed}
            \textcolor{white}{\textbf{\textsc{Inclusion}}}       & \textcolor{white}{\textbf{\textsc{Quantifier}}}         & \textcolor{white}{\textbf{\textsc{Condition}}}       \\\hline\rowcolor{backgroundColor}
            $R\aptcirc R^{\dagger}\subset \nabla_{B}$            & \Gape[0pt][2pt]{\makecell{For each $b_{1},b_{2}\in B$}} & \Gape[0pt][2pt]{\makecell{If, for each $x\in A$,\\$x\sim_{R}b_{1}$ or $x\sim_{R}b_{2}$,\\then $b_{1}\neq b_{2}$.}}  \\\rowcolor{backgroundColor}
            $\nabla_{A}           \subset R^{\dagger}\aptcirc R$ & \Gape[0pt][2pt]{\makecell{For each $a_{1},a_{2}\in A$}} & \Gape[0pt][2pt]{\makecell{If $a_{1}\neq a_{2}$, then, for each $x\in A$, \\we have $x\sim_{R}b_{1}$ or $x\sim_{R}b_{2}$.}} \\\rowcolor{black!05!backgroundColor}
            $\nabla_{B}           \subset R\aptcirc R^{\dagger}$ & \Gape[0pt][2pt]{\makecell{For each $b_{1},b_{2}\in B$}} & \Gape[0pt][2pt]{\makecell{If $b_{1}\neq b_{2}$, then, for each $y\in B$, \\we have $a_{1}\sim_{R}y$ or $a_{2}\sim_{R}y$.}} \\\rowcolor{backgroundColor}
            $R^{\dagger}\aptcirc R\subset \nabla_{A}$            & \Gape[0pt][2pt]{\makecell{For each $a_{1},a_{2}\in A$}} & \Gape[0pt][2pt]{\makecell{If, for each $y\in B$,\\$a_{1}\sim_{R}y$ or $a_{2}\sim_{R}y$,\\then $a_{1}\neq a_{2}$}} \\\hline
        \end{tabular}
    \end{center}
    \endgroup
    Equivalently:
    ...
    \begin{center}
        \begin{tabular}{|Sc|Sc|Sc|}\\\hline\rowcolor{darkRed}
            \textcolor{white}{\textbf{\textsc{Inclusion}}}       & \textcolor{white}{\textbf{\textsc{Quantifier}}}         & \textcolor{white}{\textbf{\textsc{Condition}}}       \\\hline\rowcolor{backgroundColor}
            $R\aptcirc R^{\dagger}\subset \nabla_{B}$            & \Gape[0pt][2pt]{\makecell{For each $b_{1},b_{2}\in B$}} & $(R^{-1}(b_{1})\cup R^{-1}(b_{2})=A)\implies(b_{1}\neq b_{2})$ \\\rowcolor{backgroundColor}
            $\nabla_{A}           \subset R^{\dagger}\aptcirc R$ & \Gape[0pt][2pt]{\makecell{For each $a_{1},a_{2}\in A$}} & $(a_{1}\neq a_{2})\implies(R(a_{1})\cup R(a_{2})=B)$           \\\rowcolor{black!05!backgroundColor}
            $\nabla_{B}           \subset R\aptcirc R^{\dagger}$ & \Gape[0pt][2pt]{\makecell{For each $b_{1},b_{2}\in B$}} & $(b_{1}\neq b_{2})\implies(R^{-1}(b_{1})\cup R^{-1}(b_{2})=A)$ \\\rowcolor{backgroundColor}
            $R^{\dagger}\aptcirc R\subset \nabla_{A}$            & \Gape[0pt][2pt]{\makecell{For each $a_{1},a_{2}\in A$}} & $(R(a_{1})\cup R(a_{2})=B)\implies(a_{1}\neq a_{2})$           \\\hline
        \end{tabular}
    \end{center}
    \endgroup
...
\begin{Proof}{Proof of \cref{isomorphisms-and-equivalences-in-rel-apartness}}%
    ...
    \begin{itemize}
        \item\SloganFont{\cref{isomorphisms-and-equivalences-in-rel-apartness-1}$\iff$\cref{isomorphisms-and-equivalences-in-rel-apartness-2}: }This follows from the fact that $\aptsfbfRel$ is locally posetal, so that natural isomorphisms and equalities of $1$-morphisms in $\sfbfRel$ coincide.

...
\begin{motivation}{Setting for Internal Right Kan Extensions in $\sfbfRel$}{setting-for-internal-right-kan-extensions-in-rel}%
    ...
    \[
        \begin{tikzcd}[row sep={5.0*\the\DL,between origins}, column sep={5.0*\the\DL,between origins}, background color=backgroundColor, ampersand replacement=\&]
            \&%
            B%
            \arrow[d, "\Ran_{R}(S)", densely dashed for mid vert, mid vert]%
            \\%
            A%
            \arrow[ru, "R",mid vert]%
            \arrow[r, "S"',""'{name=S,pos=0.425},mid vert]%
            \&%
            X\mrp{.}%
            % 2-Arrows
            \arrow[from=1-2,to=S,shorten=0.75*\the\DL,Rightarrow,shift left=0.15*\the\DL]%
        \end{tikzcd}
    \]%
    These will form a functor
    \[
        \Ran_{R}%
        \colon%
        \eRel(A,X)%
...
\begin{proposition}{Internal Right Kan Extensions in $\sfbfRel$}{internal-right-kan-extensions-in-rel}%
    ...
    \begin{enumerate}
        \item\label{internal-right-kan-extensions-in-rel-1}Viewing relations from $A$ to $C$ as subsets of $A\times C$, we have
            \[
                \Ran_{R}(S)%
                =%
                \{%
                    ...
                    \begin{aligned}
                        ...
                        &\text{then we have $a\sim_{S}c$}%
                    \end{aligned}
                \}.%
            \]%
        \item\label{internal-right-kan-extensions-in-rel-2}Viewing relations as functions $A\times B\to\TV$, we define
            \begin{align*}
                (S\intcirc R)^{-_{1}}_{-_{2}} &\defeq \int_{a\in A}\eHom_{\TTV}(R^{-_{2}}_{a},S^{-_{1}}_{a})\\
                                              &=      \bigwedge_{a\in A}\eHom_{\TTV}(R^{-_{2}}_{a},S^{-_{1}}_{a}),%
            \end{align*}
            where the meet $\bigwedge$ is taken in the poset $(\TV,\preceq)$ of \ChapterRef{\ChapterSets, \cref{sets:the-poset-of-truth-values}}{\cref{the-poset-of-truth-values}}.

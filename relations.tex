...
\begin{definition}{Relations}{relations}%
    ...
    \end{enumerate}
\end{definition}
\begin{Proof}{Proof of \cref{relations}}%
    We will prove that \cref{relations-1,relations-2,relations-3,relations-4,relations-5} are indeed equivalent in just a moment.
\end{Proof}
...
\begin{remark}{Unwinding \cref{relations-2}, \rmII}{unwinding-relations-2}%
    ...
    \begin{align*}
        \{\text{relations from $A$ to $B$}\} &\defeq \mathcal{P}(A\times B)\\
...
\begin{Proof}{Proof of \cref{relations}}%
    ...
    \begin{itemize}
        ...
        \item\SloganFont{\cref{relations-2}$\iff$\cref{relations-5}: }This follows from the universal property of the powerset $\mathcal{P}(X)$ of a set $X$ as the free cocompletion of $X$ via the characteristic embedding
            ...
            %--- Begin Footnote ---%
            \footnote{%
                Note that by \ChapterRef{\ChapterConstructionsWithSets, \cref{constructions-with-sets:unwinding-the-direct-image-function-associated-to-a-function}}{\cref{unwinding-the-direct-image-function-associated-to-a-function}}, we have $\Lan_{\chi}(f)=f_{*}$, the direct image of $f$.
                \par\vspace*{\TCBBoxCorrection}
            }%
            %---  End Footnote  ---%
            % BEGIN RAW HTML %

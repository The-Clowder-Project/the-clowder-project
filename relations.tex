...
Let $A$, $B$, and $X$ be sets and let $R\colon A\rightproarrow B$ be a relation.
\begin{motivation}{Setting for Internal Right Kan Extensions in $\sfbfRel$}{setting-for-internal-right-kan-extensions-in-rel}%
    ...
    \[
        \begin{tikzcd}[row sep={5.0*\the\DL,between origins}, column sep={5.0*\the\DL,between origins}, background color=backgroundColor, ampersand replacement=\&]
            \&%
            B%
            \arrow[ru, "\Ran_{R}(S)"', bend right=10]%
            \\%
            A%
            \arrow[ru, "R"]%
            \arrow[r, "S",""{name=S}]%
    ...
    \]%
\end{motivation}
\begin{definition}{Intersection Composition of Relations}{intersection-composition-of-relations}%
    The \index[set-theory]{relation!intersection composition of}\textbf{intersection composition of $R$ and $S$} is the relation \index[notation]{SafterR@$S\intcirc R$}$S\intcirc R$ defined as follows:%
    \begin{enumerate}
        \item\label{composition-of-relations-1}Viewing relations from $A$ to $C$ as subsets of $A\times C$, we define
            \[
                S\intcirc R

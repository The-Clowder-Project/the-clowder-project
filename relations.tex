...
\begin{definition}{Composition of Relations}{composition-of-relations}%
    ...
    \end{enumerate}
  \end{definition}
  \begin{Proof}{Proof of equivalence of \cref{composition-of-relations}}%
    Omitted.
  \end{Proof}
...
\begin{Proof}{Proof of \cref{properties-of-graphs-of-functions}}%
    ...
    \begin{enumerate}
      \item \SloganFont{Preservation of Identities. } For $A$ a set, $a,b \in A$ we have
            \[ a \sim_{\Gr(\id_A)} b \iff \id_A a = b \iff a = b \iff a \sim_{\chi_A} b, \]
            so $\Gr(\id_A) = \chi_A.$
      \item \SloganFont{Preservation of composition. } For $A, B, C$ sets, $f \colon A \to B, g \colon B \to C, a \in A, c \in C$ we have
            \begin{align*}
...
\begin{explanation}{Monomorphisms in $\sfRel$}{explanation-monomorphisms-in-rel}%
    ...
    \begin{webcompile}
        ...
        \begin{tikzcd}[row sep={3.0*\the\DL,between origins}, column sep={8.0*\the\DL,between origins}, background color=backgroundColor, ampersand replacement=\&,%
                execute at end picture={%
                    ...
                    % B
                    \draw (\tikzcdmatrixname-3-2) [partial ellipse=0:162:{2.5*\the\DL} and {8.0*\the\DL}];
                    \draw (\tikzcdmatrixname-3-2) [partial ellipse=166:177.75:{2.5*\the\DL} and {8.0*\the\DL}];
                    \draw (\tikzcdmatrixname-3-2) [partial ellipse=182.5:199.25:{2.5*\the\DL} and {8.0*\the\DL}];
                    \draw (\tikzcdmatrixname-3-2) [partial ellipse=205:226:{2.5*\the\DL} and {8.0*\the\DL}];
    ...
    \begin{enumerate}
        ...
        \item\label{explanation-monomorphisms-in-rel-3}$R$ doesn't need to be surjective, so $B$ can have elements that aren't in the image of $R$.
    \end{enumerate}
\end{explanation}

...
\end{definition}
\begin{Proof}{Proof of \cref{the-associator-of-the-product-of-sets}}%
...
\end{definition}
\begin{Proof}{Proof of \cref{the-left-unitor-of-the-product-of-sets}}%
    \FirstProofBox{Invertibility}%
...
\end{definition}
\begin{Proof}{Proof of \cref{the-right-unitor-of-the-product-of-sets}}%
    \FirstProofBox{Invertibility}%
...
\end{definition}
\begin{Proof}{Proof of \cref{the-symmetry-of-the-product-of-sets}}%
    \FirstProofBox{Invertibility}%
...
\begin{definition}{The Diagonal of $\times$}{the-diagonal-of-the-product-of-sets}%
    ...
    \begin{webcompile}
        ...
        \begin{tikzcd}[row sep={5.0*\the\DL,between origins}, column sep={4.0*\the\DL,between origins}, background color=backgroundColor, ampersand replacement=\&]
            ...
            \&
            \Sets\times\Sets\mrp{,}
            \arrow[ru,"times"'{pos=0.55},bend right=10]
            \&
            % 2-Arrows
    ...
    for each $x\in X$.
\end{definition}
\begin{Proof}{Proof of \cref{the-diagonal-of-the-product-of-sets}}%
    We need to show that, given a function $f\colon X\to Y$, the diagram
    \[
...
\begin{Proof}{Proof of \cref{properties-of-the-diagonal-map}}%
    ...
    \begin{enumerate}
        \item\SloganFont{\cref{properties-of-the-diagonal-map-monoidality-compatibility-with-strong-monoidality-constraints}: Compatibility With Strong Monoidality Constraints: }We need to show that the diagram
            \[
                \begin{tikzcd}[row sep={5.0*\the\DL,between origins}, column sep={10.0*\the\DL,between origins}, background color=backgroundColor, ampersand replacement=\&]
                    X\times Y
                    \arrow[r,"\Delta_{X}\times\Delta_{Y}"]
        ...
        \item\SloganFont{\cref{properties-of-the-diagonal-map-monoidality-compatibility-with-strong-unitality-constraints}: Compatibility With Strong Unitality Constraints: }As shown in the proof of \cref{the-left-unitor-of-the-product-of-sets}, the inverse of the left unitor of $\Sets$ with respect to to the product at $X\in\Obj(\Sets)$ is given by
            \[
                \LUnitor^{\Sets,-1}_{X}(x)%
                \defeq%
                (\point,x)%
...
\begin{theorem}{The Universal Property of $(\Sets,\times,\pt)$}{the-universal-property-of-sets-times-pt}%
    ...
    \begin{enumerate}
        \item\label{the-universal-property-of-sets-times-pt-existence-of-an-internal-hom}\SloganFont{Existence of an Internal Hom. }The product
            \[
                \times%
                \colon
                \Sets\times\Sets
        ...
        \item\label{the-universal-property-of-sets-times-pt-the-unit-object-is-pt}\SloganFont{The Unit Object Is $\pt$. }We have $\Unit_{\Sets}=\pt$.
    \end{enumerate}
    More precisely, the full subcategory of the category $\ModuliCategory^{\cld}_{\E_{\infty}}(\Sets)$ of \cref{TODO} spanned by the closed symmetric monoidal categories $\left(\phantom{\mrp{\LUnitor^{\Sets}}}\Sets\right.$, $\otimes_{\Sets}$, $[-_{1},-_{2}]_{\Sets}$, $\Unit_{\Sets}$, $\LUnitor^{\Sets}$, $\RUnitor^{\Sets}$, $\left.\sigma^{\Sets}\right)$ satisfying \cref{the-universal-property-of-sets-times-pt-existence-of-an-internal-hom,the-universal-property-of-sets-times-pt-the-unit-object-is-pt} is strongly contractible (i.e.\ isomorphic to the punctual category).
\end{theorem}
\begin{Proof}{Proof of \cref{the-universal-property-of-sets-times-pt}}%
    ...
    \[
        ...
        \Sets%
    \]%
    admits a \emph{unique} symmetric closed monoidal functor structure
    \begin{gather*}
        \id^{\otimes}_{\Sets}       \colon A\otimes_{\Sets}B \isorightarrow A\times B,\\
        \id^{\Hom}_{\Sets}          \colon [A,B]_{\Sets}     \isorightarrow \Sets(A,B),\\
        \id^{\otimes}_{\Unit|\Sets} \colon \Unit_{\Sets}     \isorightarrow \pt
    \end{gather*}
    making it into a symmetric monoidal strongly closed isomorphism of categories from $\left(\phantom{\mrp{\LUnitor'}}\Sets\right.$, $\otimes_{\Sets}$, $[-_{1},-_{2}]_{\Sets}$, $\Unit_{\Sets}$, $\LUnitor'$, $\RUnitor'$, $\left.\sigma'\right)$ to the closed symmetric monoidal category $\left(\phantom{\mrp{\LUnitor^{\Sets}}}\Sets\right.$, $\times$, $\Sets(-_{1},-_{2})$, $\Unit_{\Sets}$, $\LUnitor^{\Sets}$, $\RUnitor^{\Sets}$, $\left.\sigma^{\Sets}\right)$ of $\Sets$ and products of \cref{the-monoidal-structure-on-sets-associated-to-the-product}.
    ...
    \[
        ...
        \end{tikzcd}
    \]%
    commutes as well. Now, let $A\in\Obj(\Sets)$, let $a\in A$, and consider the diagram
    \begin{webcompile}
        \scalebox{0.575}{%
            \begin{tikzcd}[row sep={0.0*\the\DL,between origins}, column sep={0.0*\the\DL,between origins}, background color=backgroundColor, ampersand replacement=\&]
    ...
    \[
        \begin{tikzcd}[row sep={5.0*\the\DL,between origins}, column sep={7.5*\the\DL,between origins}, background color=backgroundColor, ampersand replacement=\&]
            A\otimes_{\Sets}B
            \arrow[r,"\sigma'_{A,B}"]
            \arrow[d,"\id^{\otimes}_{\Sets|A,B}"']
            \&
            B\otimes_{\Sets}A
            \arrow[d,"\id^{\otimes}_{\Sets|B,A}"]
            \\
            A\times B
            \arrow[r,"\sigma^{\Sets}_{A,B}"']
            \&
            B\times A
        \end{tikzcd}
    \]%
    ...
    \[
        \begin{tikzcd}[row sep={5.0*\the\DL,between origins}, column sep={8.0*\the\DL,between origins}, background color=backgroundColor, ampersand replacement=\&]
            \pt\otimes_{\Sets}\pt
            \arrow[r,"\sigma'_{\pt,\pt}"]
            \arrow[d,"\id^{\otimes}_{\Sets|\pt,\pt}"']
            \&
            \pt\otimes_{\Sets}\pt
            \arrow[d,"\id^{\otimes}_{\Sets|\pt,\pt}"]
            \&[-1.0*\the\DL]
    ...
    \begin{align*}
        \LUnitor'_{B} &= \LUnitor^{\Sets}_{B}\circ\phi_{\pt,B},\\
        \LUnitor'_{B} &= \LUnitor^{\Sets}_{B}\circ\psi_{\pt,B}.
    \end{align*}
    Postcomposing both sides by $\LUnitor^{\Sets,-1}_{B}$ gives
    \begin{align*}
        \LUnitor^{\Sets,-1}_{B}\circ\LUnitor'_{B} &= \phi_{\pt,B},\\
        \LUnitor^{\Sets,-1}_{B}\circ\LUnitor'_{B} &= \psi_{\pt,B},
    \end{align*}
    and thus we have
    \[
        \phi_{\pt,B}%
...
\begin{corollary}{A Second Universal Property for $(\Sets,\times,\pt)$}{a-second-universal-property-for-sets-times-pt}%
    The symmetric monoidal structure on the category $\Sets$ of \cref{the-monoidal-structure-on-sets-associated-to-the-product} is uniquely determined by the following requirements:
    \begin{enumerate}
        \item\label{a-second-universal-property-for-sets-times-pt-sided-preservation-of-colimits}\SloganFont{Two-Sided Preservation of Colimits. }The product
            \[
                \times%
                \colon
                \Sets\times\Sets
        ...
        \item\label{a-second-universal-property-for-sets-times-pt-the-unit-object-is-pt}\SloganFont{The Unit Object Is $\pt$. }We have $\Unit_{\Sets}=\pt$.
    \end{enumerate}
...
\end{definition}
\begin{Proof}{Proof of \cref{the-associator-of-the-coproduct-of-sets}}%
...
\end{definition}
\begin{Proof}{Proof of \cref{the-left-unitor-of-the-coproduct-of-sets}}%
...
\end{definition}
\begin{Proof}{Proof of \cref{the-right-unitor-of-the-coproduct-of-sets}}%
...
\begin{definition}{The Symmetry of $\icoprod$}{the-symmetry-of-the-coproduct-of-sets}%
    ...
    for each $(x,y)\in X\times Y$.
\end{definition}
\begin{Proof}{Proof of \cref{the-symmetry-of-the-coproduct-of-sets}}%
